...
impl State {
    ...
    pub fn print_verbose(&mut self) {
        ...
        println!("Registers");
        if self.pc.get_altered() {println!("  PC: {} | {:#06x}", self.pc.get(), self.pc.get())}
        if self.ir.get_altered() {println!("  IR: {} | {:#06x}", self.ir.get(), self.ir.get())}
        for i in 0..16 {
            if self.r[i].get_altered() {println!("  R{}: {} | {:#06x}", i, self.r[i].get(), self.r[i].get())}
        }
