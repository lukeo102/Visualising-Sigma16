        ...
        match token {
            Ok(token) => match token {
                ...
                Tokens::Data(args) => {
                    ...
                    if let Some(value) = extracted.name("data") { // If data
                        let name = extracted["name"].to_string();

                        if let Some(value) = extracted.name("hex") { // Match hex
                            assembled.push(u16::from_str_radix(&value.as_str()[1..], 16).unwrap());
                            
                        } else if let Some(value) = extracted.name("const") { // Match constant
                            assembled.push(value.as_str().parse::<u16>().unwrap());
                            
                        } else if let Some(value) = extracted.name("var") { // Match variable
                            assembled.push(0_u16);

                            data_inserts.entry(value.as_str().parse().unwrap())
                                .or_default()
                                .push(cursor as u16);
                        }
                        data_locations.insert(name, cursor as u16);
                        cursor += 1;
                    } else { // else jump label
...
fn parse_irxargs(args: &str, cursor: usize, data_inserts: &mut HashMap<String, Vec<u16>>) -> (u16, u16) {
    regex!(
    ...
    if let Some(cons) = extarcted_args.name("cons") { // If constant in addr

        addr = cons.as_str().parse::<u16>().unwrap();

    } else if let Some(var_match) = extarcted_args.name("var_match") { // If variable in addr
        let var = var_match.as_str();
        // If variable mapping exists, add to the vec, otherwise create mapping
        data_inserts.entry(var.parse().unwrap())
            .or_default()
            .push(cursor as u16);

    } else if let Some(hex) = extarcted_args.name("hex") { // If hex in addr
...
fn parse_jump(command: String, data_inserts: &mut HashMap<String, Vec<u16>>, cursor: usize) -> (u16, u16) {
    regex!(REGEX = r"(?P<type>jump(?:[a-zA-Z]{2})?) +(?:(?P<label>[A-z][A-Za-z0-9]*)|(?P<const>\$[A-Fa-f0-9]{4}))(?:\[R(?P<register>[0-9]|1[0-5])])?");
    ...
    let address = 
        if let Some(addr) = extracted_command.name("label") {
            data_inserts.entry(addr.as_str().parse().unwrap())
                .or_default()
